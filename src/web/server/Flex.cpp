...
namespace Jde::Web{
	...
	α Server::InitListener( typename tcp::acceptor::rebind_executor<executor_with_default>::other& acceptor, const tcp::endpoint& endpoint )ι->bool{
    beast::error_code ec;
...
namespace Server{
	...
	α Server::Send( IRestException&& e, sp<RestStream> stream )ι->void{
		auto res = e.Response();
		stream->AsyncWrite( move(res) );
	}
