...
#define let const auto
	...
	{
		let p = dynamic_cast<const T*>( &parent );
#ifdef _MSC_VER
	...
	struct BinaryOperation : virtual IValue, ValuesBlock
	{
		...
		STATIC_PARAMS override{ vector<string> v{ A.StaticParams(type), B.StaticParams(type) }; v.erase( std::remove_if(v.begin(), v.end(), [](let&x){return x.empty();}), v.end() ); return Str::AddSeparators(v, ", "); }
...
#pragma warning( default : 4250 )
	...
	struct Ternary final: IValue, ValuesBlock
	{
		...
		STATIC_PARAMS override{ vector<string> v{ If().StaticParams(type), Then().StaticParams(type), Else().StaticParams(type) }; v.erase( std::remove_if(v.begin(), v.end(), [](let&x){return x.empty();}), v.end() ); return Str::AddSeparators(v, ", "); }
	...
	{
		optional<T> v;
		for( let* c = e.FirstChildElement(T::ElementName); c; c = c->NextSiblingElement(T::ElementName) )
		{
	...
	{
		let field = Blockly::Factory<Blockly::Field>( e );
		THROW_IF( field.Name!="OP", "({})Expecting single field named OP.", Id );
		let operation = std::distance( blocklyStrings.begin(), std::find(blocklyStrings.begin(), blocklyStrings.end(), field.Value) );
