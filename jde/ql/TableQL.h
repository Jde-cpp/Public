...
namespace Jde::QL{
	struct ColumnQL final{
		...
		string JsonName;
		mutable const DB::Column* SchemaColumnPtr{nullptr};
	};
	...
	struct MutationQL final{
		...
		α Input(SRCE)Ε->json;
		template<class T=uint> auto Id( ELogTags tags=ELogTags::None, SRCE )Ε->T{ return Json::Getε<T>( Args, "id", tags | ELogTags::QL, sl ); }
