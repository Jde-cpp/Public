...
namespace Jde::DB::Sqlite::OpcProcs{
	α RegisterOpcVariantInsert()ι->void{
		RegisterProc( "opc_variant_insert", []( sqlite3& db, const vector<Value>& params, RowΛ* onRow, SL sl )->uint{
			EnsureDataTypeNodeId( db, params[0], sl );
			let y = ExecuteStatement( db, "insert into opc_variants( data_type_id, array_dims ) values( ?, ? )", {params[0], params[1]}, nullptr, sl );
			if( onRow )
