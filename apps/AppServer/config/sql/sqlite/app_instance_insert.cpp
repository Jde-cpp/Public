...
namespace Jde::DB::Sqlite::AppProcs{
	...
	α RegisterAppInstanceInsert( IProcs& procs )ι->void{
		procs.RegisterProc( "app_instance_insert", [&procs]( sqlite3& db, const vector<Value>& params, RowΛ* onRow, SL sl )->uint{
			let instanceId = InstanceInsert( procs, db, params[0], params[1], params[2], sl );
			if( onRow )
