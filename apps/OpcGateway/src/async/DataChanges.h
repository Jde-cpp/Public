...
namespace Jde::Opc::Gateway{
	...
	struct ΓOPC DataChangeAwait final : TAwait<FromServer::SubscriptionAck>{
		...
		DataChangeAwait( flat_set<ExNodeId>&& nodes, sp<IDataChange>&& dataChange, sp<UAClient>&& c, SRCE )ι:base{sl}, _nodes{move(nodes)}, _dataChange{move(dataChange)}, _client{move(c)}{}
	...
	private:
		...
		flat_map<ExNodeId, MonitoredItemCreateResult> _existingNodes;
	};

	//Ξ DataChangesSubscribe( flat_set<ExNodeId> nodes, sp<IDataChange> socketSession, sp<UAClient> c )ι{ return DataChangeAwait{ move(nodes), move(socketSession), move(c) }; }
