...
namespace Jde::DB::Sqlite::GatewayProcs{
	α RegisterGatewayServerConnectionInsert()ι->void{
		RegisterProc( "gateway_server_connection_insert", []( sqlite3& db, const vector<Value>& params, RowΛ* onRow, SL sl )->uint{
			if( !params[4].is_null() && params[4].get_number<uint>()!=0 )
				ExecuteStatement( db, "update gateway_server_connections set is_default=0", {}, nullptr, sl );
