create or alter proc [dbo].gateway_server_connection_insert @name varchar(255), @target varchar(255), @attributes smallint, @description varchar(2047), @is_default bit, @certificate_uri varchar(2047), @url varchar(2047), @server_connection_id int output as begin
	set nocount on;
  if @is_default=1
    update [dbo].gateway_server_connections set is_default=0;
  insert into [dbo].gateway_server_connections( name, attributes, created, target, description, certificate_uri, is_default, url )
