...
create procedure gateway_server_connection_insert( _name varchar(255),_target varchar(255),_attributes smallint,_description varchar(2047), _is_default bit, _certificate_uri varchar(2047),_url varchar(2047), out _server_connection_id int unsigned ) begin
	if( _is_default ) then
		update gateway_server_connections set is_default=0;
	end if;
	insert into gateway_server_connections( name,attributes,created,target,description,certificate_uri, is_default, url )
