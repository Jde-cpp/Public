...
#buildRelativePath supplies the src->lib/exe rename first.  Echoes "<repoSourceDir> <buildDir>" on success,
#the failure reason on non-zero return.
function makefileDirForFile() (
	...
	if [[ ${file#"$repoSourceDir/"} == "$file" ]]; then
	...
	if [[ $buildDir == $buildRoot ]]; then
		echo "no Makefile for ${file#"$repoSourceDir/"} under $buildRoot - not a configured source dir.";
		return 1;
	fi;
	echo "$repoSourceDir $buildDir";
)
function buildProject() (
	...
	read repoSourceDir buildDir <<< "$resolved";
	echo repoSourceDir=$repoSourceDir, buildRoot=$buildRoot, buildDir=$buildDir;
