-- The role's own permissions - the is_role=0 members access_role_add minted for it - go with it.  A member that is
...
create or alter proc [dbo].access_role_purge( @role_id int )as begin
	declare @members table( permission_id int primary key );
	insert into @members
	select m.member_id
	...
	where m.role_id=@role_id
		and p.is_role=0
		and not exists( select 1 from access_role_members o where o.member_id=m.member_id and o.role_id<>@role_id )
		and not exists( select 1 from access_acl a where a.permission_id=m.member_id );

	delete from access_acl where permission_id=@role_id;
	delete from access_role_members where role_id=@role_id or member_id=@role_id;
	delete from access_permission_rights where permission_id=@role_id or permission_id in ( select permission_id from @members );
	delete from access_roles where role_id=@role_id;
	delete from access_permissions where permission_id=@role_id or permission_id in ( select permission_id from @members );
end
