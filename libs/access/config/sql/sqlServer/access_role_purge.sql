create or alter proc [dbo].access_role_purge( @role_id int )as begin
	delete from access_acl where permission_id=@role_id or permission_id in ( select member_id from access_role_members where role_id=@role_id );
	delete from access_role_members where member_id=@role_id;
