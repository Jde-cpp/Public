...
//	params: [0]=_provider_type_id, [1]=_slug; out _provider_id returned as the result row.
namespace Jde::DB::Sqlite::AccessProcs{
	α RegisterAccessProviderInsert( IProcs& procs )ι->void{
		RegisterInsertProc( procs, "access_provider_insert", "insert into access_providers( provider_type_id, slug ) values( ?, ? )", 2 );
	}
}
