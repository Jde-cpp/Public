...
namespace Jde::DB::Sqlite::AccessProcs{
	α RegisterAccessUserInsertKey( IProcs& procs )ι->void{
		procs.RegisterProc( "access_user_insert_key", [&procs]( sqlite3& db, const vector<Value>& params, RowΛ* onRow, SL sl )->uint{
			...
			let y = procs.ExecuteStatement( db, "insert into access_users( identity_id, modulus, exponent, issuer, subject_alt, distinguished, expiration ) values( ?, ?, ?, ?, ?, ?, ? )", {Value{identityId}, params[0], params[1], params[6], params[7], params[8], params[10]}, nullptr, sl );
			if( onRow )
