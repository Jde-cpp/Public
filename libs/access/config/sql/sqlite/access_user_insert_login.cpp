...
namespace Jde::DB::Sqlite::AccessProcs{
	α RegisterAccessUserInsertLogin()ι->void{
		RegisterProc( "access_user_insert_login", []( sqlite3& db, const vector<Value>& params, RowΛ* onRow, SL sl )->uint{
			Value providerId = params[1];
			optional<string> providerName;
			if( !params[2].is_null() ){
				if( let id = ScalarUInt(db, "select provider_id from access_providers where target=?", {params[2]}, sl); id ) //mysql `select into` keeps _provider_id when no row matches.
					providerId = Value{*id};
				...
				ExecuteStatement( db, "select name from access_provider_types where provider_type_id=?", {params[1]}, &nameRow, sl );
			}
			let providerTarget = providerName ? *providerName+"-"+params[0].get_string() : params[0].get_string();
			let identityId = IdentityInsert( db, params[0], providerId, Value{providerTarget}, Value{}, Value{}, Value{false}, sl );
			let y = ExecuteStatement( db, "insert into access_users( identity_id, login_name ) values( ?, ? )", {Value{identityId}, params[0]}, nullptr, sl );
			if( onRow )
