...
namespace Jde::Opc{
	...
	α UAClient::StateCallback( UA_Client *ua, UA_SecureChannelState channelState, UA_SessionState sessionState, StatusCode connectStatus )ι->void{
		...
		if( sessionState == UA_SESSIONSTATE_ACTIVATED || connectStatus==UA_STATUSCODE_BADIDENTITYTOKENINVALID || connectStatus==UA_STATUSCODE_BADCONNECTIONREJECTED || connectStatus==UA_STATUSCODE_BADINTERNALERROR || connectStatus==UA_STATUSCODE_BADUSERACCESSDENIED || connectStatus==UA_STATUSCODE_BADSECURITYCHECKSFAILED || connectStatus == UA_STATUSCODE_BADIDENTITYTOKENREJECTED ){
			_awaitingActivation.erase_if( [ua, sessionState,connectStatus]( sp<UAClient> pClient){
				...
				let id{ make_tuple(pClient->Target(),pClient->UserId) };
	...
	α UAClient::Retry( function<void(sp<UAClient>&&, HCoroutine&&)> f, UAException e, sp<UAClient> pClient, HCoroutine h )ι->ConnectAwait::Task{
		//TODO limit retry attempts.
		let id{ make_tuple(pClient->Target(), pClient->UserId) }; let password = pClient->Password;
		{
			ul _{ _clientsMutex };
		...
		if( e.Code==UA_STATUSCODE_BADCONNECTIONCLOSED || e.Code==UA_STATUSCODE_BADSERVERNOTCONNECTED ){
			try{
				pClient = co_await GetClient(get<0>(id), get<1>(id), password);
	...
	α UAClient::SetMonitoringMode( Opc::SubscriptionId subscriptionId )ι->void{
	...
	α UAClient::DataSubscriptionDelete( Opc::SubscriptionId subscriptionId, flat_set<MonitorId>&& monitoredItemIds )ι->void{
	...
	α UAClient::Find( str id, str userId, str pw )ι->sp<UAClient>{
		sl _{ _clientsMutex };
		sp<UAClient> y;
		if( auto p = id.size() ? _clients.find(make_tuple(id,userId)) : find_if( _clients, [&](let& c){return c.second->IsDefault() && c.second->UserId==userId; } ); p!=_clients.end() && p->second->Password==pw )
			y = p->second;
		return y;
	}
