...
struct Γ StrPair
{
   ...
   bool Empty() const{ return _start == _end; }
    ...
    void SetStr( sv str, int flags=0 );

    α ParseText( char* in, const char* endTag, Mode mode, uint& line )->char*;
...
constexpr inline StrPair::Mode operator|=(StrPair::Mode& a, StrPair::Mode b){ return a = (StrPair::Mode)((uint)a | (uint)b); }
...
template<Str::IsString T=string> α UnEscape( Str::bsv<typename T::traits_type> xml )ι->T;
Φ Close( sv xml )ε->string;

namespace XMLUtil
{
   Ξ IsUTF8Continuation( const char p )->bool{ return ( p & 0x80 ) != 0; }
   Ξ IsWhiteSpace( char p )->bool{ return !IsUTF8Continuation(p) && isspace( static_cast<unsigned char>(p) ); }// Anything in the high order range of UTF-8 is assumed to not be whitespace. This isn't correct, but simple, and usually works.
   Ξ SkipWhiteSpace( const char* p, uint* curLineNumPtr )->const char*
   {
        TIXMLASSERT( p );
   ...
   Ξ SkipWhiteSpace( char* const p, uint* curLineNumPtr )->char* {
    ...
    static Ξ StringEqual( sv p, sv q, bool insensitive )->bool
	 {
		 return insensitive ? ToIV(p)==ToIV(q) : p==q;
	 }
...
struct Γ XMLNode
{
   ...
   void SetValue( sv val/*, bool staticMem=false*/ );
    ...
    */
   α FirstChildElement( sv n = {} )Ι->const XMLElement*;
   α FirstChildElement( sv name = {} )ι->XMLElement*{ return const_cast<XMLElement*>(const_cast<const XMLNode*>(this)->FirstChildElement( name )); }
   α FirstChildElementRef( sv n = {} )Ε->const XMLElement&{ auto p = FirstChildElement( n ); THROW_IF( !p, "Could not find element '{}'", n ); return *p; }
    ...
    */
    ...
    α NextSibling()Ι->const XMLNode*{ return _next; }
    α NextSibling()ι->XMLNode*{ return _next; }

    /// Get the next (right) sibling element of this node, with an optionally supplied name.
    //α NextSiblingElement( sv name )Ι->const XMLElement*;
	 α NextSiblingElement( sv name = {} )Ι->const XMLElement*;
    α NextSiblingElement( sv name = {} )ι->XMLElement*{ return const_cast<XMLElement*>( const_cast<const XMLNode*>(this)->NextSiblingElement(name) ); }
...
private:
    ...
    void SetName( sv name );

    char* ParseDeep( char* p, XMLDocument& doc );
...
struct Γ XMLElement : public XMLNode
{
    ...
    void SetName( char* str/*, bool staticMem=false*/ ){ SetValue2( str ); }
    void SetName( sv str/*, bool staticMem=false*/ ){ SetValue( str/*, staticMem*/ ); }
    ...
    */
    ...
    /// Sets the named attribute to value.
    void SetAttribute( sv name, unsigned value )		{
        XMLAttribute* a = FindOrCreateAttribute( name );
        a->SetAttribute( value );
    }

	/// Sets the named attribute to value.
	void SetAttribute(sv name, int64_t value) {
		XMLAttribute* a = FindOrCreateAttribute(name);
		a->SetAttribute(value);
	}

    /// Sets the named attribute to value.
    void SetAttribute(sv name, uint64_t value) {
        XMLAttribute* a = FindOrCreateAttribute(name);
        a->SetAttribute(value);
    }

    /// Sets the named attribute to value.
    void SetAttribute( sv name, bool value )			{
        XMLAttribute* a = FindOrCreateAttribute( name );
        a->SetAttribute( value );
    }
    /// Sets the named attribute to value.
    void SetAttribute( sv name, double value )		{
        XMLAttribute* a = FindOrCreateAttribute( name );
        a->SetAttribute( value );
    }
    /// Sets the named attribute to value.
    void SetAttribute( sv name, float value )		{
        XMLAttribute* a = FindOrCreateAttribute( name );
        a->SetAttribute( value );
    }
*/
   ...
   {
      let s = TryChildText( elementName );
		let trim = Jde::Str::Trim( s );
...
private:
    ...
    XMLAttribute* FindOrCreateAttribute( sv name );
...
{
    XMLDocument( sv value, bool insensitive, bool fix=false, SRCE )ε;
    XMLDocument( bool processEntities = true, Whitespace whitespaceMode = PRESERVE_WHITESPACE, bool insensitive=false, bool fix=false );
    ~XMLDocument();
    ...
    */
    α RootElement()ι->XMLElement*{ return FirstChildElement(); }
    α RootElement()Ι->const XMLElement*{ return const_cast<XMLElement*>(FirstChildElement()); }
    ...
    */
    XMLElement* NewElement( sv name );
    /**
    	Create a new Comment associated with
    	this Document. The memory for the Comment
    	is managed by the Document.
    */
    XMLComment* NewComment( sv comment );
    /**
    	Create a new Text associated with
    	this Document. The memory for the Text
    	is managed by the Document.
    */
    XMLText* NewText( sv text );
    /**
    	...
    	@endverbatim
    */
    XMLDeclaration* NewDeclaration( sv text={} );
    /**
    	Create a new Unknown associated with
    	this Document. The memory for the object
    	is managed by the Document.
    */
    XMLUnknown* NewUnknown( sv text );
   ...
   α Identify( char* p, const XMLNode& parent )ι->tuple<char*,XMLNode*>;
    ...
    α Line()ι->uint&{ return _parseCurLineNum; }
    const bool Fix;
private:
...
*/
   ...
   template<Str::IsString T> α XMLNode::HtmlText( bool trim, bool unescape )Ι->tuple<T,const XMLNode*>
   {
	   ...
	   if( let* p = ToText(); p )
      {
         ...
         if( auto n = Next(true); n && ToIV(n->Name())=="br" && n->NextSibling() )
         {
            ...
            if( n->NextSibling() )
            {
               let [txt,pNode2] = n->NextSibling()->HtmlText<T>();
               y+=txt;
               pNode = pNode2;
            }
            else
	   ...
	   {
		   for( let* c = FirstChild(); c; c = c->NextSibling() )
		   {
			   if( c->ToComment() )
				   continue;
			   if( let* p{c->ToText()}; p )
				   y+=p->Value<view>();
			   else if( let p{c->ToElement()}; p && p->Value<iv>()=="BR" )
				   y+='\n';
			   else if( let p{c->IsHtmlStyle() ? c->ToElement() : nullptr}; p )
   ...
   template<Str::IsView T> α XMLElement::Text()Ι->T
   {
      ...
      return node && node->ToText() ? node->Value<T>() : T{};
   }
