...
#define $ std::basic_string<char, TT>
	...
	α Toε( const X& x, ELogLevel l=ELogLevel::Debug, SRCE )ε->Y{
		Y y;
		let e=std::from_chars( x.data(), x.data()+x.size(), y );
		if( e.ec!=std::errc{} )
	...
	namespace Str{
		...
		Ŧ AddSeparators( T collection, sv separator, bool quote=false )ι->string;//TODO - remove
		Ŧ AddCommas( T value, bool quote=false )ι{ return AddSeparators( value, ",", quote ); }//TODO - remove
		Ŧ Join( T collection, sv separator=",", bool quote=false )ι->string{ return AddSeparators( collection, separator, quote ); }
	...
	namespace Internal{
		...
		template<Str::IsStringLike T, Str::IsStringLike TCriteria> α FindPhraseT( tuple<vector<T>,vector<uint>> x, const std::vector<TCriteria>& criteria )ι->optional<Str::FindPhraseResult>{
			...
			for( auto p=words.begin(); (p=std::find(p, words.end(), firstWord))!=words.end(); ++p ){
				let iStartWord{ (uint)std::distance(words.begin(), p) }; uint i = iStartWord;
				bool equal = true;
				uint j = 1;
				for( ; equal && i<words.size()-1 && j<criteria.size(); ++j )
					equal = words[++i]==T{ criteria[j].data(), criteria[j].size() };
				if( let iStart = locations[iStartWord]; /*equal && (*/!y || iStart<y->Start || j>y->NextEntry/*)*/ ){
...
#undef TSV
		...
		template<class T=sv, bool TStem=false> α WordsLocation( Str::bsv<TT> x )ι->tuple<vector<T>,vector<uint>>{ //[words, endIndex]
			...
			for( uint iStart{0}, iEnd; iStart<x.size(); iStart=iEnd ){
				...
				if( ch>0xff )
					iEnd-=2;
				if( let length{iEnd-iStart}; length ){
