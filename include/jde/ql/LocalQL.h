...
namespace Jde::QL{
	...
	struct LocalQL final : IQL{
		...
		α Query( string query, UserPK executer, bool returnRaw=true, SRCE )ε->up<TAwait<jvalue>> override{ return mu<QLAwait<jvalue>>( move(query), executer, _schemas, returnRaw, sl ); }
		α QueryObject( string query, UserPK executer, bool returnRaw=true, SRCE )ε->up<TAwait<jobject>> override{ return mu<QLAwait<jobject>>( move(query), executer, _schemas, returnRaw, sl ); }
		α QueryArray( string query, UserPK executer, bool returnRaw=true, SRCE )ε->up<TAwait<jarray>> override{ return mu<QLAwait<jarray>>( move(query), executer, _schemas, returnRaw, sl ); }
