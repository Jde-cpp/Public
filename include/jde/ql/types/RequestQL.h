...
namespace Jde::QL{
	...
	struct RequestQL{
		...
		α Mutations( this auto&& self )ι->auto&{ ASSERT(self._value.index()==1); return get<1>(self._value); }//deducing this: const and non-const in one body.
		...
		α Queries( this auto&& self )ι->auto&{ ASSERT(self._value.index()==0); return get<0>(self._value); }
		α ToString()Ι->string;
	private:
