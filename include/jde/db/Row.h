...
namespace Jde::DB{
	struct Row final/*: IRow*/ {
		...
		α GetInt32( uint i )Ι->int32_t{ return _values[i].Get<int32_t>(); }
		α GetInt32Opt( uint i )Ι->optional<int32_t>{ return IsNull(i) ? optional<int32_t>{} : _values[i].Get<int32_t>(); }
		α GetInt( uint i )Ι->int64_t{ return _values[i].Get<int64_t>(); }
		α GetIntOpt( uint i )Ι->optional<int64_t>{ return IsNull(i) ? optional<int64_t>{} : _values[i].Get<int64_t>(); }
		...
		α GetUInt16Opt( uint i )Ι->optional<uint16_t>{ return IsNull(i) ? optional<uint16_t>{} : _values[i].Get<uint16_t>(); }
		α GetUInt32( uint i )Ε->uint32_t{ return static_cast<uint32_t>(GetUInt(i)); }
		α GetUInt32Opt( uint i )Ι->optional<uint32_t>{ return IsNull(i) ? optional<uint32_t>{} : _values[i].Get<uint32_t>(); }
