...
import { arraysEqual, ChipTones, cloneClassArray, DetailPage, Flex, IGraphQL, Properties, QLSelector, Style, TableSettings, TargetRow, toIdArray, ViewFieldSettings } from 'jde-framework';
...
export const userTableSettings:TableSettings = {
 ...
 //not locked down.  Hence the shape of this page:  no Add and no detail route (the natural key is schema+target+criteria,
 //which `:target` alone cannot address), no Purge - the resources table advertises only Delete and Subscribe as grantable
 ...
 //read on the user's Effective rights tab, and here it only ever looked like a duplicate of its table (same target, no
