...
declare -A webFiles;
if isStale Web.FromServer || [ $clean == 1 ]; then webFiles[Web.FromServer]=web_from_server; fi;
echo 'Creating web proto files';
create $JDE_BASH/libs/web/client/proto webFiles $wsDir;
echo 'Created web proto files';
declare -A appFiles;
if isStale Log || [ $clean == 1 ]; then appFiles[Log]=log; fi;
if isStale App.FromClient || [ $clean == 1 ]; then appFiles[App.FromClient]=app_from_client; fi;
if isStale App.FromServer || [ $clean == 1 ]; then appFiles[App.FromServer]=app_from_server; fi;
if isStale App || [ $clean == 1 ]; then appFiles[App]=app; fi;
if isStale Common || [ $clean == 1 ]; then appFiles[Common]=common; fi;
