//ProfileStore splits its two writers: save() is the server row through the 'IProfileService' token, deduped against a
//10-entry LRU of JSON strings (parse-per-load so callers never share mutable objects), and set() is the browser-local
//copy in localStorage.  load() reads the server, falling back to localStorage logged out or on failure.
...
describe( 'ProfileStore', ()=>{
	...
	it( 'logged out: load falls back to localStorage and save is a no-op', async ()=>{
		...
		expect( localStorage.getItem('k') ).toBe( JSON.stringify({a:1}) );//save is server-only: it leaves the local copy alone
